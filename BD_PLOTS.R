################################################################################
# PLOT IRF TO BELIEF DISTORTION SHOCKS (reduced-form methodology)
################################################################################

N_vars_plot <- N_vars
y_names_plot <- c(y_names)
y_dnames_plot <- c(y_dnames)
...
for (xc in 1:N_vars_plot) {
  ...
  for (bc in 1:slct_bootruns) {
    columns_list[[bc]] <- SAVE_RESULTS[[slct_robust]][[slct_method]]$irf_boot[[bc]][, xc] # Extract column bc corresponding to bc-th bootstrap iteration.
  }
...
alpha <- 1-slct_ci        # Select significance level (Default: 5%).
alpha_2 <- (1-slct_ci)*2  # Report two different significance levels (second default: 10%).

for (xc in 1:N_vars_plot) {
  # Include CI at the 5% level:
  ...
  for (tc in 1:(irf_periods+1) ) {
    # Include CI at the 5% level:
    IRF_CI[[xc]][tc,1] <-  as.numeric( quantile( irf_vals[[xc]][tc,] , 1-alpha/2 ,na.rm = TRUE) )  # Find Upper-Bound CI (ie, 0.975).
    IRF_CI[[xc]][tc,2] <- as.numeric( quantile( irf_vals[[xc]][tc,] , alpha/2 ,na.rm = TRUE) )   # Find Lower-Bound CI (ie, 0.025).
    
    # Include CI at the 10% level:
    IRF_CI_10[[xc]][tc,1] <-  as.numeric( quantile( irf_vals[[xc]][tc,] , 1-alpha_2/2 ,na.rm = TRUE) )  # Find Upper-Bound CI (ie, 0.975).
    IRF_CI_10[[xc]][tc,2] <- as.numeric( quantile( irf_vals[[xc]][tc,] , alpha_2/2 ,na.rm = TRUE) )   # Find Lower-Bound CI (ie, 0.025).
    }
}
rm(xc,tc)
...
for (i in 1:N_vars_plot){
  ...
  temp_plot <- temp_plot + scale_fill_distiller() #Distill colors.
...
for (i in 1:N_vars_plot){
  temp_plot <- ggplot(data = dt_irf[ORDER==i & PERIOD<=irf_periods_plot]) +
    ...
    scale_y_continuous(labels = scales::number_format(accuracy = 0.1),
                       limits = c(ifelse(i==1,-0.4, ifelse(i==2, -3 , ifelse(i==4, -0.8, NA ) ) ),
                                  ifelse(i==1, 1.15, ifelse(i==2, 0.1 , ifelse(i==4, 1.3, NA) ) ) ) ) + # Format y-axis labels
    labs(title = ifelse(i==1,"Reduced Form"," "), y = "", x = "")  + 
    theme(axis.text = element_text(size = 14),
          axis.title = element_text(size = 14),
          plot.title = element_text(size = 14, hjust = 0.5))
