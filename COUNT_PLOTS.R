################################################################################
# PLOT IRF FROM COUNTERFACTUAL POLICIES (reduced-form methodology)
################################################################################
...
figure_comb <- do.call(ggarrange, c(combined_plot, list(ncol = 2, nrow=3) )  )  # Combine all plots in one figure.
figure_comb
...
for (i in show_plots){
  temp_plot <- ggplot(data = dt_irf_2[ORDER==i & PERIOD<=irf_periods_plot]) +
    ...
    scale_y_continuous(labels = scales::number_format(accuracy = 0.1),
                       limits = c(ifelse(i==1,-0.5, ifelse(i==2, -0.4 , ifelse(i==4, -0.2, NA ) ) ),
                                  ifelse(i==1, 1.15, ifelse(i==2, 0.1 , ifelse(i==4, 0.7, NA) ) ) ) ) + # Format y-axis labels
    labs(title = ifelse(i==1,"Reduced Form"," "), y = "", x = "", color = "Counterfactuals:",linetype = "Counterfactuals:", shape = "Counterfactuals:", fill = "Counterfactuals:") +
    scale_shape_manual(values = slct_shape)+
    scale_color_manual(values = slct_color) +
    scale_fill_manual(values = slct_color) +
    scale_linetype_manual(values = slct_linetype) +
